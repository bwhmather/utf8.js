;(function() {
    ...
    test("ascii", function() {
        ...
        deepEqual( utf8.encode("\x00\x01\x13\x25\x37\x49\x5b\x6d\x7f"),
                   new Uint8Array([0x00, 0x01, 0x13, 0x25, 0x37,
                               0x49, 0x5b, 0x6d, 0x7f]),
                   "lowest to highest");
    });
    ...
    test("two byte characters", function() {
        deepEqual( utf8.decode(new Uint8Array([0xc2, 0x80])),
                   "\u0080",
                   "lowest two byte character");

        deepEqual( utf8.decode(new Uint8Array([0xdf, 0xbf])),
                   "\u07ff",
                   "highest two byte character");

        raises(function () {
                utf8.decode(new Uint8Array([0xc1, 0xbf]));
            }, Error, "raises on unneeded first byte in two byte character");

        raises(function () {
                utf8.decode(new Uint8Array([0xcf]));
            }, Error, "raises on truncated two byte character");

    });

    test("three byte characters", function() {
        deepEqual( utf8.decode(new Uint8Array([0xe0, 0xa0, 0x80])),
                   "\u0800",
                   "lowest three byte character");

        deepEqual( utf8.decode(new Uint8Array([0xef, 0xbf, 0xbf])),
                   "\uffff",
                   "highest three byte character");

        raises(function () {
                utf8.decode(new Uint8Array([0xe0, 0x9f, 0xbf]));
            }, Error, "raises on unneeded first byte in three byte character");

        raises(function () {
                utf8.decode(new Uint8Array([0xef, 0xbf]));
            }, Error, "raises on truncated three byte character");
    });

    test("four byte characters", function() {
        deepEqual( utf8.decode(new Uint8Array([0xf0, 0x9d, 0x84, 0x9e])),
                   "\ud834\udd1e",
                   "G clef");

        raises(function () {
                utf8.decode(new Uint8Array([0xf0, 0x8f, 0xbf, 0xbf]));
            }, Error, "raises on unneeded first byte in four byte character");

        raises(function () {
                utf8.decode(new Uint8Array([0xf7, 0xbf, 0xbf]));
